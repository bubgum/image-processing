...
def distance():
    try:
        ...
        cursor = db.cursor()
        cursor.execute("DELETE FROM canbera_sobel")
        db.commit()
        

        db = MySQLdb.connect("localhost","root","zosso","citra" )
        cursor = db.cursor()
        cursor.execute("SELECT * FROM data_train_sobel")
        data_train =  list(cursor)
            
        cursor = db.cursor()
        cursor.execute("SELECT * FROM data_test_sobel WHERE no=(SELECT MAX(no) FROM data_test_sobel);")
        ...
        for x in range(len(data_train)) :
            ...
            for y in range(82):                
                ...
                print canbera
                index2 += 1
            query = "INSERT INTO canbera_sobel \
...
#            print canbera
        ...
        sql = "SELECT canbera_sobel.result, data_train_sobel.img_source FROM canbera_sobel JOIN data_train_sobel ON canbera_sobel.id_train = data_train_sobel.no ORDER BY canbera_sobel.result ASC LIMIT 10"
        cursor.execute(sql)
        row = list(cursor)
        index=0
