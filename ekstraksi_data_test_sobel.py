...
sql = "SELECT file_name FROM upload_sobel WHERE id=(SELECT MAX(id) FROM upload_sobel);"
cursor.execute(sql)
row = cursor.fetchone()
while row is not None:
    path = 'C:/Ampps/www/citra/upload_sobel/' + str(row)[2:-3]    
    row = cursor.fetchone()
...
def input_data():
    ...
    query = "INSERT INTO data_test_sobel " 
