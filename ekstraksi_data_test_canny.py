...
sql = "SELECT file_name FROM upload_canny WHERE id=(SELECT MAX(id) FROM upload_canny);"
cursor.execute(sql)
row = cursor.fetchone()
while row is not None:
    path = 'C:/Ampps/www/citra/upload_canny/' + str(row)[2:-3]    
    row = cursor.fetchone()
...
def input_data():
    ...
    query = "INSERT INTO data_test_caanny " 
