...
def distance():
    try:
        ...
        cursor = db.cursor()
        cursor.execute("DELETE FROM canbera")
        db.commit()
        ...
        cursor = db.cursor()
        cursor.execute("SELECT * FROM data_test WHERE no=(SELECT MAX(no) FROM data_test);")
        ...
        for x in range(len(data_train)) :
            ...
            for y in range(82):                
                ...
                print canbera
                index2 += 1
            query = "INSERT INTO canbera \
                    VALUES (%s, %s, %f) " % (data_test[0][0], data_train[index1][0], canbera)
            db = MySQLdb.connect("localhost","root","zosso","citra" )
...
#            print canbera
        ...
        sql = "SELECT canbera.result, data_train.img_source FROM canbera JOIN data_train ON canbera.id_train = data_train.no ORDER BY canbera.result ASC LIMIT 10"
        cursor.execute(sql)
        row = list(cursor)
        index=0
